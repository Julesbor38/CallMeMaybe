...
class ConstrainedDecoder:
    ...
    def _next_logits(self, input_ids: list[int]) -> list[float]:
        """Return next-token logits."""
        return self.llm.get_logits_from_input_ids(input_ids)

    def allowed_tokens(self, candidates: list[list[int]],
                       generated: list[int]) -> set[int]:
    ...
    def filter_candidates(self,
                          ...
                          generated: list[int]) -> list[list[int]]:
        """Keep only candidates matching the generated prefix."""

        return [candidate for candidate in candidates
                if candidate[: len(generated)] == generated]

    def best_allowed_token(self,
                           logits: list[float], allowed: set[int]) -> int:
        """Return the highest-scoring allowed token."""

        if not allowed:
            raise RuntimeError("No allowed token.")

        return max(allowed, key=lambda token: logits[token])

    def decode_choice(self, prompt: str, choices: list[str]) -> str:
        """Decode one choice using constrained decoding."""

        encoded_choices = [self._encode(choice) for choice in choices]
        generated: list[int] = []
        prompt_ids = self._encode(prompt)

        while True:
            allowed = self.allowed_tokens(encoded_choices, generated)
            if not allowed:
                raise RuntimeError("No valid continuation.")

            context = prompt_ids + generated
            logits = self._next_logits(context)
            next_token = self.best_allowed_token(logits, allowed)
            generated.append(next_token)
            encoded_choices = self.filter_candidates(
                encoded_choices, generated)
            if (len(encoded_choices) == 1 and generated == encoded_choices[0]):
    ...
    def expected_parameters(
         self, function: FunctionDefinition) -> list[tuple[str, str]]:
    ...
    def generate_arguments(self, prompt: str,
                           function: FunctionDefinition) -> dict[str, Any]:
        """Generate the arguments for a function."""
        return self.decode_json(prompt, function)
    ...
    def decode_number(self, context: list[int],) -> tuple[float, list[int]]:
        ...
        digit_tokens = {self._encode(str(i))[0] for i in range(10)}
        allowed = digit_tokens | {self._encode("-")[0], self._encode(".")[0]}
        while True:
            logits = self._next_logits(context + generated)
            next_token = max(allowed, key=lambda token: logits[token],)
            generated.append(next_token)
            logits = self._next_logits(context + generated)
            best = max(
                range(len(logits)), key=lambda token: logits[token])
    ...
    def decode_string(self, context: list[int],) -> tuple[str, list[int]]:
        ...
        while True:
            logits = self._next_logits(context + generated)
            next_token = max(range(len(logits)),
        ...
        start = decoded.find('"')
        if start == -1:
            raise RuntimeError("Opening quote not found.")
        end = decoded.find('"', start + 1)
        if end == -1:
            raise RuntimeError("Closing quote not found.")
        value = decoded[start + 1:end]
        return value, generated

    def decode_boolean(self, prompt: str) -> tuple[bool, list[int]]:
        """Decode a boolean and return both the value and its tokens."""
        value = self.decode_choice(prompt, ["true", "false"])
        return (value == "true", self._encode(value))

    def decode_json(self, prompt: str,
                    function: FunctionDefinition) -> dict[str, Any]:
